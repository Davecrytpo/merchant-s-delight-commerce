...
export default function ReviewSection({ productId }: { productId: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user) return;
    try {
      await submitReview.mutateAsync({ productId, rating, comment });
      setComment("");
      setRating(5);
      setShowForm(false);
    } catch (err) {
          ...
          <div className="flex items-center gap-4">
            <div className="flex">
              {Array.from({ length: 5 }).map((_, i) => (
                <Star key={i} className={`w-4 h-4 ${i < Math.floor(reviews?.reduce((acc, r) => acc + r.rating, 0) / (reviews?.length || 1)) ? "text-primary fill-primary" : "text-muted"}`} />
              ))}
            </div>
            <span className="text-sm text-muted-foreground">{reviews?.length || 0} reviews based on recent purchases</span>
          </div>
        </div>
        {!showForm && user && (
          <button 
        ...
        <div className="glass p-6 md:p-8 rounded-3xl animate-in fade-in slide-in-from-top-4 duration-300">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              ...
              className="w-full gold-gradient text-background font-bold py-4 rounded-2xl flex items-center justify-center gap-2 shadow-lg shadow-primary/20 transition-all disabled:opacity-50"
            >
